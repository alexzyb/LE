...
- [ ] 修改 `src/config.py`：加 `DATABASE_URL` / `BG_EXPORT_DIR` 环境变量 + `pellet_silo_level` 阈值
- [ ] 修改 `src/data.py`：查询 `live_data` 表 + 显示时过滤 + SQLite/PostgreSQL 双支持 + `get_date_range()`
- [ ] 在 SQLite 中创建 `live_data` 表（由 ingest.py 首次运行自动建表）

### Phase 8b — 采集引擎
- [ ] 创建 `src/ingest.py`：CSV 尾部增量读取 → 基本校验 → 30s 重采样 → pivot 宽表 → UPSERT（不做范围清洗）
- [ ] 创建 `ingest_state.json` 状态管理（字节偏移追踪 + 崩溃恢复）
- [ ] 用 `BaumgartnerData/*.csv` 样本数据测试，验证宽表列名正确

### Phase 8c — Dashboard 对接 + 页面重组
- [ ] 修改 `src/app.py`：默认时间范围自动适配 + 页面布局重组（见 §9）
- [ ] Overview 页: 移除 Energy/Dryer/Quality section, 新增 Mill Status + Pellet Silo 液位罐 + Dispatch
- [ ] Detail 页: 移除 Dryer/Quality/CHP/Downtime panel, 新增 Pellet Silo + Dispatch panel
- [ ] 修改 `src/charts.py`：fig_pellet_silos() 改百分比液位罐 + 新增 fig_silo_level_trend/fig_dispatch_trend + 缺失列保护
- [ ] 修改 `src/translations.py`：加 Pellet Silo + Dispatch + "No data" 翻译 key
